...
function newCheck(value, name) {
  ...
  function addAssertion(assertionName, proto, args) {
    ...
    Object.defineProperty(assertion, 'condition', {
      get: function() {
        throw new Error('.condition is write-only');
      },
  ...
  function addOperator(operatorName, proto) {
    ...
    Object.defineProperty(operator, 'apply', {
      get: function() {
        throw new Error('.apply is write-only');
      },
      set: function(apply) {
        nodsl.check(typeof apply === 'function', '.apply must be a function; got ', apply);
        if (operator instanceof BinaryOperator) {
          priv.state.syntax.addBinaryOperator(apply);
        } else {
          priv.state.syntax.addUnaryOperator(apply);
        }
      },
    });
