...
function groupByVariableName(context, retVal, assertion) {
  var name = assertion.getValue.name(context);
  var current = retVal.length === 0? createGroup(assertion): retVal.pop();
  var currentName = current.getValue.name(context);
  if (name !== currentName) {
    retVal.push(current);
...
function groupToString(context, group) {
  ...
  var name = group.getValue.name(context);
  var conditions = group.serialized.join(' ');
  var value = group.getValue.value(context);
