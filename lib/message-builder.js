...
function groupByVariableName(context, retVal, assertion) {
  ...
  appendOperators(assertion.operators, current.message);
...
function appendOperators(operators, message) {
  var operatorStr = operatorsToString(operators);
  if (operatorStr.binary) {
    message.push(operatorStr.binary);
  }
  if (operatorStr.unary) {
    message.push(operatorStr.unary);
  }
}
...
function unknownToString(unknown) {
...
//      return '\''+ unknown +'\'';
    case 'function':
      return functionToString(unknown);
    case 'object':
      return objectToString(unknown);
  }
}

function functionToString(func) {
  return func.name? 'function[name=' + func.name +']' : 'unnamed function';
}
...
function shallowObjectToString(object) {
  if (object === null) {
    return 'null';
  }
  if (object instanceof Array) {
...
//      return '\''+ value +'\'';
    ...
    case 'object':
      return shallowObjectToString(value);
  }
}
