...
function bindProperties(pub, getValues, priv) {
  getValues.forEach(function(getValue) {
    Object.defineProperty(pub, getValue.name, {
      get: getValue.bind(null, priv),
      set: function() { throw new Error('property '+ getValue.name +' is read-only'); },
    });
  });
}
