...
function ExpressionStack() {
  ...
  var pub = {};
  bindMethods(pub, [push, pop, forcePop, popWhenReady, addOperand, addUnaryOperator, addBinaryOperator, evaluate], priv);
...
function addUnaryOperator(priv, operator) {
  priv.current.syntax.addUnaryOperator(operator);
}

function addBinaryOperator(priv, operator) {
  priv.current.syntax.addBinaryOperator(operator);
}
