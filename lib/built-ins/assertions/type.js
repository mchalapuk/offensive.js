...
module.exports = {
  ...
  'anArray': new Assertion(function(context) {
    this.message.appendString('an array');
  ...
  'anInstanceOf': new ParameterizedAssertion(function(context, RequiredClass) {
    ...
    if (RequiredClass.name) {
      this.message.appendString('an instance of '+ RequiredClass.name);
    } else {
      this.message.appendString('an instance of %UNNAMED_CLASS%');
    }
...
function typeofAssertion(requiredType) {
  ...
  return new Assertion(function() {
    this.message.appendString(getTypePrefix(requiredType) + requiredType);
