...
module.exports = {
  ...
  'elementThatIs': new ParameterizedAssertion(function(context, index, assertName, condition) {
    ...
    this.getValue = Getters.element(index);
  ...
  'eachElementIs': new ParameterizedAssertion(function(context, assertName, condition) {
    ...
    var StackLevel = {
      ROOT: 'eachElementIs '+ assertName +'root',
      ELEM: 'eachElementIs '+ assertName +'root',
    };

    context._push(StackLevel.ROOT);
    if (!context.is.anArray._result) {
      context._pop(StackLevel.ROOT);
      return;
    }
    context._reset();
    context._push(StackLevel.ELEM);
    ...
    context._value.forEach(function(element, index) {
      ...
      context._pop(StackLevel.ELEM);
      noop(context._operatorContext.and);
      context._push(StackLevel.ELEM);
    });

    context._forcePop(StackLevel.ELEM);
    context._pop(StackLevel.ROOT);
  }),
